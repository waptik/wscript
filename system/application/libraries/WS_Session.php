...
// 		 * It can either set to GMT or time(). The pref
		...
		{	
			...
			if (($this->userdata['last_activity'] + $this->object->config->item('sess_time_to_update')) < $this->now)
			{
				$this->sess_update();
			}
		}
	...
	{	
		...
		else
		{
			...
			if (($row->last_activity + $this->sess_length) < $this->now) 
			{
				...
				$session = @unserialize(base64_decode($row->user_data));
				if ( ! is_array($session) ) {
					$session = array();
				}
	...
	{		
		...
		(
			...
			'uri_string'	=> $this->object->uri->uri_string ()
		);
		...
		unset($ud['user_agent']);
		unset($ud['ip_address']);
		$query_array['user_data'] = base64_encode(serialize($ud));
