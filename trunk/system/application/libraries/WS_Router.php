...
class WS_Router {
	...
	function _validate_request ( $segments ) {
		...
		$full_s = implode ( '', $segments );
		$new_s = '';
		
		if ( preg_match ( '/^(.*)-(.*)\.html$/', $full_s, $matches ) ) {
			if ( isset ( $matches [ 2 ] ) ) {
				$new_s = 'wallpapers/show/' . $matches [ 2 ];
				$new_s_a = explode ( '/', $new_s );
				$s = $new_s_a;
				$this->uri->segments = $s;
				return $s;
			}
		...
		elseif ( preg_match ( '#^(.*)-(.*)$#', $full_s, $matches ) ) {
			if ( isset ( $matches [ 2 ] ) ) {
				$new_s = 'categories/show/' . $matches [ 2 ];
				$new_s_a = explode ( '/', $new_s );
				$s = $new_s_a;
				$this->uri->segments = $s;
				return $s;
